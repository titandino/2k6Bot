...
public class Bot {
	...
	public static boolean FILTER_GAME = false;
			...
			for (int y = 0; y < 104; y++) {
				...
				if (groundItems != null) {
					for (Item item = (Item) groundItems.peekLast(); item != null; item = (Item) groundItems.reverseGetNext()) {
						if (ItemDef.forID(item.ID) != null && ItemDef.forID(item.ID).name != null && !ItemDef.forID(item.ID).name.toLowerCase().contains(id.toLowerCase()))
							continue;
	...
	public static boolean processCommand(String string) {
		...
		} else if (cmd[0].startsWith("filter")) {
			FILTER_GAME = !FILTER_GAME;
			printConsole("Filtering game messages: "+FILTER_GAME);
