...
public final class signlink implements Runnable {
	...
	public static String findcachedir() {
		boolean exists = (new File((new StringBuilder(String.valueOf(System
				.getProperty("user.home")))).append("/project2006016/").toString()))
				.exists();
		if (exists) {
			return (new StringBuilder(String.valueOf(System
					.getProperty("user.home")))).append("/project2006016/").toString();
		} else {
			File f = new File((new StringBuilder(String.valueOf(System
					.getProperty("user.home")))).append("/project2006016/").toString());
			f.mkdir();
			return (new StringBuilder(String.valueOf(System
					.getProperty("user.home")))).append("/project2006016/").toString();
				...
				if (waveplay) {
					...
					try {
						audioInputStream = AudioSystem
								.getAudioInputStream(new File(wave/* soundFile */));
	...
	public static final void reporterror(String s) {
		...
		try {
			...
			s = s.replace('&', '_');
			s = s.replace('#', '_');
			DataInputStream datainputstream = openurl("reporterror" + 317
					+ ".cgi?error=" + errorname + " " + s);
