...
public final class TextDrawingArea extends DrawingArea {
	...
	public void method380(String s, int i, int j, int k) {
		drawText(j, s, k, i - method384(s));
	}

	public void drawCenteredText(int i, String s, int k, int l) {
		drawText(i, s, k, l - method384(s) / 2);
	}
	...
	public void method387(int i, String s, int j, int k, int l) {
		...
		for (int i1 = 0; i1 < s.length(); i1++) {
			char c = s.charAt(i1);
			if (c != ' ')
				method392(aByteArrayArray1491[c], i + anIntArray1494[c] + (int) (Math.sin((double) i1 / 5D + (double) j / 5D) * 5D), k + anIntArray1495[c] + (int) (Math.sin((double) i1 / 3D + (double) j / 5D) * 5D), anIntArray1492[c], anIntArray1493[c],
						l);
			i += anIntArray1496[c];
		}
